...
class Git {
    ...
    async getFirstCommit() {
        const list = await exec_promise_1.default("git", ["rev-list", "--max-parents=0", "HEAD"]);
        return list.split("\n").pop();
    }
    ...
    async publish(releaseNotes, tag, prerelease = false) {
    ...
    async getTagNotInBaseBranch(branch, options = {}) {
        const baseTags = (await this.getTags(`origin/${this.options.baseBranch}`)).reverse();
